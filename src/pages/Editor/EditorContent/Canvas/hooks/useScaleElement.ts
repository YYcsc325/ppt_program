...
type OperateResizeHandlersTypes = keyof typeof OperateResizeHandlers;
...
const getOppositePoint = (
  direction: OperateResizeHandlersTypes,
...
export default (
  ...
  const scaleElement = (
    ...
    const fixedRatio =
      ctrlOrShiftKeyActive || ('fixedRatio' in element && element.fixedRatio);
    ...
    // 其中线条和被旋转过的元素不参与吸附对齐
      ...
      for (const el of elementList.current) {
  ...
  const scaleMultiElement = (
    ...
    const originElementList: PPTElement[] = JSON.parse(
      JSON.stringify(elementList.current),
    );
