...
export default () => {
  ...
  const addElementsFromClipboard = (elements: PPTElement[]) => {
    ...
    const currentSlideElementIdList = currentSlide.elements.map((el) => el.id);
    ...
    store.addElement(elements);
    store.setActiveElementIdList(Object.values(elIdMap));
    addHistorySnapshot();
  };
