...
export default () => {
  ...
  const themeColor = store.storeData.theme.themeColor;
  const fontColor = store.storeData.theme.fontColor;
  const fontName = store.storeData.theme.fontName;
  ...
  const createElement = (element: PPTElement) => {
    ...
    store.setActiveElementIdList([element.id]);

    if (creatingElement) store.setCreatingElement(null as any);
